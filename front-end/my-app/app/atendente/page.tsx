  ...
  async function assumirSolicitacao(id: number) {
    try {
      const response = await fetch(
        `http://localhost:8080/solicitacoes/${id}/assumir`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
      ...
      if (!response.ok) throw new Error("Erro ao assumir solicitação");
    ...
    <main className="flex h-screen bg-gray-900">
      ...
      <div className="flex-1 flex flex-col bg-gray-100">
        <header className="bg-white px-8 py-4 border-b border-gray-200">
          <h1 className="text-2xl font-bold text-gray-900">Gestão de Protocolos</h1>
        </header>
        ...
        <div className="flex-1 p-8 overflow-auto">
          ...
          <div className="grid grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-xl shadow p-6">
              ...
              <p className="text-3xl font-bold text-blue-600">{protocolos.length}</p>
            </div>
            <div className="bg-white rounded-xl shadow p-6">
              <h2 className="text-gray-500">Novos</h2>
              <p className="text-3xl font-bold text-red-600">
                {protocolos.filter((p) => p.status === "NOVO").length}
              </p>
            </div>
            <div className="bg-white rounded-xl shadow p-6">
              <h2 className="text-gray-500">Em andamento</h2>
              <p className="text-3xl font-bold text-yellow-600">
                {protocolos.filter((p) => p.status === "EM_ANDAMENTO").length}
              </p>
            </div>
            <div className="bg-white rounded-xl shadow p-6">
              <h2 className="text-gray-500">Concluídas</h2>
              <p className="text-3xl font-bold text-green-600">
                {protocolos.filter((p) => p.status === "CONCLUIDA").length}
              </p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white p-10 rounded-xl text-center">
              <p className="text-red-600 font-semibold mb-3">Erro ao carregar solicitações.</p>
              <button
              ...
              {protocolos.map((item) => (
                <div key={item.id} className="bg-white rounded-xl shadow p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h2 className="font-bold text-lg">{item.protocolo}</h2>
                      <p className="text-gray-600">{item.servicoNome}</p>
                  ...
                  <div className="mb-4">
                    ...
                    <p className="text-gray-700">{item.cidadaoNome}</p>
                  </div>

                  <div className="mb-5">
                    <p className="font-semibold">Atendente</p>
                    <p className="text-gray-700">{item.atendenteNome || "Não atribuído"}</p>
                  </div>

                  {!item.atendenteNome && (
                    <button
                      onClick={() => assumirSolicitacao(item.id)}
